...
@hydra.main(config_path = "conf", config_name = "header_extraction")
def main(cfg: DictConfig):
    ...
    for datum in tqdm(dataset):
        prompt = prompter.headerExtractionPrompt_GPT(datum)
        output = model.inference_GPT(prompt)
        post_processed_datum = prompter.processHeaderExtraction_GPT(prompt, datum, output)
