...
class TableCellHighlighter:
    ...
    def get_database_representation(self, sample_rows, sample_schema, is_outer_table=False):
        ...
        for row_id, sample_row in enumerate(sample_rows):
            sample_row_text = " | ".join([str(sample_row[i]) for i in range(len(sample_row))])
    ...
    def generate_selected_table_representation(self, selected_cell_ids, projected_cell_ids, cause_str, projected_subquery, outer_table, inner_table, projected_column_of_inner_query, deleted_columns_of_inner_query, operator, aggregate_function, deleted_subquery_str, nested_predicate_type):
        ...
        if nested_predicate_type in ["type-a", "type-ja"]:
            ...
            if isinstance([val for row_idx, val in col_val_list if val is not None][0], str):
                filtered_row_values = list(set([f"""'{val.replace("'", "''")}'""" for row_idx, val in col_val_list if val is not None]))
            else:
                filtered_row_values = list(set([f"{val}" for row_idx, val in col_val_list if val is not None]))
            ...
            if inner_values == []:
                ...
                selected_indices = self.sample_row_indices_with_max_min(col_val_list, 5, inner_values=inner_values)
        

        used_column_ids = sorted(used_column_ids)
        final_rows = [updated_rows[i] for i in selected_indices]
        final_rows = [
            [updated_rows[i][j] for j in used_column_ids]
            for i in selected_indices
        ]
        updated_schema = sample_schema
        updated_schema = [sample_schema[j] for j in used_column_ids]
        outer_table_representation = self.get_database_representation(final_rows, updated_schema, is_outer_table=True)
        outer_table_representation = f"Table Name: {outer_table}\n{outer_table_representation}"
        ...
        if nested_predicate_type in ["type-a", "type-ja"]:
            ...
            if isinstance([val for row_idx, val in col_val_list if val is not None][0], str):
                filtered_row_values = list(set([f"""'{val.replace("'", "''")}'""" for row_idx, val in col_val_list if row_idx in selected_indices and val is not None]))
            else:
                filtered_row_values = list(set([f"{val}" for row_idx, val in col_val_list if row_idx in selected_indices and val is not None]))
        ...
        inner_col_val_list = [(row_idx, row[0]) for row_idx, row in enumerate(sampled_rows)]
            ...
            for row_idx, row_values in enumerate(sampled_rows):
                if len(row_values) == 1:
                    if row_idx in sampled_indices:
                        if row_idx == sampled_indices[0] and aggregate_function.lower() == "max" and operator in [">", ">="] and nested_predicate_type == "type-a":
                            filtered_rows.append([f"{row_value} (add a predicate to filter this out)" for row_value in row_values])
                        elif row_idx == sampled_indices[0] and aggregate_function.lower() == "max" and nested_predicate_type == "type-ja":
                            filtered_rows.append([f"{row_value} (revise the logical operator from '> (Q)' to '>= (Q)')" for row_value in row_values])
                        else:
                            filtered_rows.append([f"{row_value} (selected)" for row_value in row_values])
                else:
                    if row_idx == sampled_indices[0] and aggregate_function.lower() == "max" and operator in [">", ">="] and nested_predicate_type == "type-a":
                        filtered_rows.append([f"{row_values[0]} (add a predicate to filter this out)"] + [f"{row_value}" for row_value in row_values[1:]])
                    elif row_idx == sampled_indices[0] and aggregate_function.lower() == "max" and nested_predicate_type == "type-ja":
                        filtered_rows.append([f"{row_values[0]} (revise the logical operator from '> (Q)' to '>= (Q)')"] + [f"{row_value}" for row_value in row_values[1:]])
                    elif row_idx in sampled_indices[2:]:
                        filtered_rows.append([f"{row_value}" for row_value in row_values])
            
        else:
            filtered_rows = [[row_values[0]] + [f"{row_value} (filtered out by the WHERE clause of Q)" for row_value in row_values[1:]] for row_idx, row_values in enumerate(sampled_rows) if row_idx in sampled_indices]
        
        inner_table_representation = self.get_database_representation(filtered_rows, inner_projected_columns)
        inner_table_representation = f"Table Name: {inner_table}\n{inner_table_representation}"

        return outer_table_representation + "\n\n" + inner_table_representation

    def sample_row_indices_with_max_min(self, col_val_list, n, seed=None, inner_values=[]):
        if seed is not None:
            random.seed(seed)
        ...
        if inner_values != []:
            col_val_list = [x for x in col_val_list if x[1] is not None and x[1] in inner_values]
        else:
        ...
        remaining = [row_idx for row_idx, val in col_val_list 
    ...
    def remove_join_predicate(self, where_node):
        ...
        if isinstance(pred, exp.EQ) and isinstance(pred.this, exp.Column) and isinstance(pred.expression, exp.Column):
            return pred, None
        ...
        def recurse_remove_join_predicate(expr):
            if not isinstance(expr, exp.And):
                if isinstance(expr, exp.EQ) and isinstance(expr.this, exp.Column) and isinstance(expr.expression, exp.Column):
                    return expr, None
                return None, expr
...
class ProvQueryFixer:
    ...
    def fix_query(self, query, nested_predicate_type=""):
        ...
        deleted_columns_of_inner_query = [removed_pred[1] for removed_pred in removed_preds if removed_pred[-1] == deleted_level]
        target_predicate = diff['WHERE'][deleted_level-2]
        outer_col, operator, deleted_subquery_str = self.extract_outer_column_and_subquery(target_predicate)
        ...
        try:
            ...
            deleted_subquery_execution_results = [row[0] for row in deleted_subquery_execution_results]
            ...
            if nested_predicate_type not in ["type-a", "type-ja"]:
                projected_column_of_inner_query = parse_one(deleted_subquery_str).named_selects[0]
            else:
                projected_column_of_inner_query = str(parse_one(deleted_subquery_str).selects[0].this.this)
        ...
        if outer_col is None:
            return f"Fixed query which has execution results: {subquery}", target_predicate, deleted_subquery_str, execution_result_str
        ...
        try:
            ...
            p2_sql = p2_sql.replace('_outer_outer', '_outer').replace("{'", "").replace("'}", "")
            ...
            result_representation = self.highlighter.generate_selected_table_representation(selected_cell_ids, projected_cell_ids, cause_str, projected_subquery, outer_table, inner_table, projected_column_of_inner_query, deleted_columns_of_inner_query, operator, aggregate_function, deleted_subquery_str, nested_predicate_type)
            if result_representation is None:
                return f"Fixed query which has execution results: {subquery}", target_predicate, deleted_subquery_str, execution_result_str
            return result_representation, target_predicate, deleted_subquery_str, execution_result_str
        except Exception as e:
            return f"Fixed query which has execution results: {subquery}", target_predicate, deleted_subquery_str, execution_result_str

    def find_non_empty_subquery(self, query):
        execution_result_rows, execution_result_schema = self.data_manager.sample_rows_with_sql(str(query), 10, self.rng)
        ...
        while still_empty:
            ...
            if removed_pred is not None:
                if isinstance(removed_pred, exp.Where):
                    ...
                    removed_predicates.append((str(removed_pred.this), column_name, target_depth))
                else:
                    ...
                    removed_predicates.append((str(removed_pred), column_name, target_depth))
                    ...
                    if isinstance(child, list):
                        for sub in child:
                            if isinstance(sub, exp.Expression):
                                _visit(sub, depth + 1 if isinstance(node, exp.Select) else depth)
                    elif isinstance(child, exp.Expression):
                        _visit(child, depth + 1 if isinstance(node, exp.Select) else depth)
    ...
    def remove_rightmost_predicate(self, where_node):
        ...
        if right_side and not isinstance(right_side, exp.And) and not (isinstance(right_side, exp.EQ) and isinstance(right_side.this, exp.Column) and isinstance(right_side.expression, exp.Column)):
        ...
        if right_side and isinstance(right_side, exp.EQ) and isinstance(right_side.this, exp.Column) and isinstance(right_side.expression, exp.Column):
    ...
    def remove_subquery_predicate_if_where_empty(self, parsed_ast):
        ...
        def _remove_expression_from_parent(expr_node, parent, parent_key):
            ...
            if isinstance(parent, exp.And) or isinstance(parent, exp.Or) or isinstance(parent, exp.Not):
        ...
        def _cleanup_incomplete_ands(node, parent=None, parent_key=None):
            ...
            if isinstance(node, exp.And) or isinstance(node, exp.Or) or isinstance(node, exp.Not):
        ...
        def _visit(node, parent=None, parent_key=None):
            ...
            if isinstance(node, (exp.In, exp.EQ, exp.GT, exp.GTE, exp.LT, exp.LTE, exp.Exists)):
                rhs = node.args.get("expression") or node.args.get("query") or node.args.get("expressions")
                if isinstance(rhs, exp.Subquery):
                    sub_select = rhs.this
                    if isinstance(sub_select, exp.Select):
                        if "where" not in sub_select.args or sub_select.args["where"] is None:
                            if not (len(str(parent).split(" AND ")) > 1 and str(parent).split(" AND ")[0] == str(node)):
    ...
    def diff_sql_parts(self, sql_a: str, sql_b: str) -> dict:
        ...
        select_diff = set(extract_expressions(a_expr.args.get("expressions"))) - set(extract_expressions(b_expr.args.get("expressions")))
        if select_diff:
            result["SELECT"] = list(select_diff)

        from_diff = set(extract_from(a_expr.args.get("from"))) - set(extract_from(b_expr.args.get("from")))
        if from_diff:
        ...
        where_diff = find_missing_conditions(a_expr.args.get("where"), b_expr.args.get("where"))
        if where_diff:
            result["WHERE"] = where_diff

        group_diff = set(extract_expressions(a_expr.args.get("group"))) - set(extract_expressions(b_expr.args.get("group")))
        if group_diff:
            result["GROUP BY"] = list(group_diff)

        having_diff = find_missing_conditions(a_expr.args.get("having"), b_expr.args.get("having"))
        if having_diff:
            result["HAVING"] = having_diff

        order_diff = set(extract_expressions(a_expr.args.get("order"))) - set(extract_expressions(b_expr.args.get("order")))
        if order_diff:
            result["ORDER BY"] = list(order_diff)

        a_limit = str(a_expr.args.get("limit")).strip() if a_expr.args.get("limit") else None
        b_limit = str(b_expr.args.get("limit")).strip() if b_expr.args.get("limit") else None
    ...
    def project_only_column(self, sql_query: str, column_name: str) -> str:
        ...
        if "." in column_name:
            ...
            new_projection = exp.Column(this=exp.Identifier(this=col), table=exp.Identifier(this=table))
        else:
    ...
    def fix_subquery(self, original_query: str) -> str:
        ...
        is_cor , correlated_alias = self.is_correlated_subquery_with_sqlglot(original_query)
        if not is_cor:
        ...
        if where_expr:
            def collect_conditions(expr):
                if isinstance(expr, exp.And):
                    return collect_conditions(expr.left) + collect_conditions(expr.right)
                else:
                    return [expr]
