from utils import *
from prov_query_fixer import ProvQueryFixer
from non_nested_query_generator import EGQueryGenerator
from prompts.nested import oneshotk, postorder, correction
...
class PostOrderNestedQueryGenerator(OneShotKNestedQueryGenerator):
    def __init__(self, args, data_manager, rng, all_table_set, table_info, dtype_dict, join_key_list, join_clause_list, single_query_blocks, single_query_blocks_w_agg, logger):
        super().__init__(args, data_manager, rng, all_table_set, table_info, dtype_dict, join_key_list, join_clause_list, single_query_blocks, single_query_blocks_w_agg, logger)
        ...
        self.ideal_calls += (len(self.args.hyperparams["nested_predicate_types"])-1)*self.args.num_queries*2
    
    def generate_clause(self, database_representation="", history=[], clause_type="", generated_clause_info_dict={}, relations=[], selected_inner_query_block=""):
        if clause_type == "nested_predicate":
            ...
            if "relations" not in generated_clause_info_dict:
                schema = self.get_schema(relations)
                system_prompt, user_prompt = self.get_prompt(clause_type, schema, selected_inner_query_block=[selected_inner_query_block])
                is_error, response = self.call_llm(system_prompt, user_prompt, clause_type=clause_type, max_tokens=256)
                if is_error:
                    return None, None
    ...
    def nest_inner_query_block(self,):
        ...
        for child_query_id, nested_predicate_type in enumerate(self.args.hyperparams["nested_predicate_types"]):
            nested_predicate_gen_count = 0
            while nested_predicate_gen_count < self.args.hyperparams["max_nested_predicate_gen_count"]:
                print(f"Total predicate types: {self.args.hyperparams['nested_predicate_types']}, Current predicate type: {nested_predicate_type}")
                nested_predicate_gen_count += 1
                # Select inner query block
                selected_inner_query_block, inner_table = self.select_inner_query_block(nested_predicate_type, generated_where_clause, generated_from_clause, generated_outer_tables)
                ...
                if child_query_id == 0:
                    # Generate from clause
                    candidate_outer_tables = list(set(get_joinable_tables(inner_table, self.join_clause_list) + [inner_table]))
                    from_clause, outer_tables = self.generate_clause(clause_type="from", selected_inner_query_block=selected_inner_query_block, relations=candidate_outer_tables)
                ...
                generated_nested_predicate, logical_operator = self.generate_nested_predicate(nested_predicate_type, accessed_tables, selected_inner_query_block, generated_where_clause, from_clause)
                if generated_nested_predicate is None:
                    ...
                    self.logger.error(f"Duplicate nested predicate {generated_nested_predicate}")
                    continue
                
                error_type, sql_with_nested_predicate, error_log = self.verify_nested_predicate_with_database(outer_tables, generated_nested_predicate, logical_operator, generated_where_clause)
                if self.args.is_prov:
                    stype_list = detect_type_of_nested_predicate(sql_with_nested_predicate)
                    if self.args.hyperparams["height"] > 2:
                        is_strange_type_1 = (len(stype_list) != len(self.args.hyperparams["nested_predicate_types"])+(self.args.hyperparams["height"]-1))
                        gt_type_set = set(self.args.hyperparams["nested_predicate_types"][:child_query_id+(self.args.hyperparams["height"]-1)])
                        is_strange_type_2 = (set(stype_list[:1]) != gt_type_set)
                    else:
                        is_strange_type_1 = (len(stype_list) != len(self.args.hyperparams["nested_predicate_types"][:child_query_id+1]))
                        gt_type_set = set(self.args.hyperparams["nested_predicate_types"][:child_query_id+1])
                        is_strange_type_2 = (set(stype_list) != gt_type_set)

                    if is_strange_type_1:
                        self.logger.error(f"Type mismatch for the nested predicate {generated_nested_predicate}")
                        continue
                    
                    if is_strange_type_2 or is_strange_type_1:
                        stype_set = set(stype_list)
                        self.logger.warning(f"Type mismatch for the nested predicate {generated_nested_predicate}")
                        if error_type is None:
                            error_type = "nested_predicate_type_mismatch"
                            error_log = self.get_error_log_for_nested_predicate(generated_nested_predicate, stype_set, gt_type_set)
                            generated_nested_predicate = self.correct_query(error_type, sql_with_nested_predicate, error_log, nested_predicate_type, accessed_tables, outer_tables[0])
                            if generated_nested_predicate is None:
                                self.logger.error(f"Type mismatch for the nested predicate {generated_nested_predicate}")
                                continue
                            error_type, sql_with_nested_predicate, error_log = self.verify_nested_predicate_with_database(outer_tables, generated_nested_predicate, logical_operator, generated_where_clause)
                ...
                if self.args.is_prov:
                    if error_type == "execution_error":
                        self.logger.warning(f"Execution error for the nested predicate {generated_nested_predicate}")
                        generated_nested_predicate = self.correct_query(error_type, sql_with_nested_predicate, error_log, nested_predicate_type, accessed_tables)
                    elif error_type == "empty_result_error":
                        self.logger.warning(f"Empty result error for the nested predicate {generated_nested_predicate}")
                        generated_nested_predicate = self.correct_query(error_type, sql_with_nested_predicate, error_log, nested_predicate_type)
                    
                    if generated_nested_predicate is None:
                        self.logger.error(f"Error correcting the nested predicate {generated_nested_predicate}")
                        continue
                ...
                if error_type == "execution_error":
                    self.logger.error(f"Error verifying the nested predicate with the database {generated_nested_predicate}")
                    continue
                elif error_type == "empty_result_error":
                    self.logger.error(f"Empty result error for the nested predicate {generated_nested_predicate}")
                    continue
                ...
                if len(outer_tables) > 1:
                    generated_from_clause = "FROM " + (generate_from_clause_for_full_outer_view(self.args, relations=outer_tables, join_clause_list=self.join_clause_list, is_inner_join=True))
                else:
        ...
        return generated_where_clause[len("WHERE"):].strip(), list(set(generated_outer_tables)), generated_from_clause    

    def select_inner_query_block(self, nested_predicate_type, generated_where_clause, generated_from_clause, generated_outer_tables):
        if generated_where_clause == "":
            selected_inner_query_block = str(self.get_candidate_inner_query_blocks(nested_predicate_type=nested_predicate_type)[0])
            ...
            if nested_predicate_type in ["type-a", "type-ja"]:
                ...
                for outer_table in generated_outer_tables:
                    joinable_tables = get_joinable_tables(outer_table, self.join_clause_list)
                    candidate_inner_query_tables.extend(joinable_tables)
            
            schema = self.get_schema(generated_outer_tables)
            candidate_inner_query_blocks = self.get_candidate_inner_query_blocks(nested_predicate_type=nested_predicate_type, tables=candidate_inner_query_tables, candidate_num=self.args.hyperparams["candidate_num"])
            system_prompt, user_prompt = self.get_prompt("inner_query_block", schema, generated_from_clause=[generated_from_clause], generated_where_clause=[generated_where_clause], candidate_inner_query_blocks=candidate_inner_query_blocks)
            is_error, response = self.call_llm(system_prompt, user_prompt, clause_type="inner_query_block", max_tokens=256)
            if is_error:
                return None, None
    ...
    def get_candidate_inner_query_blocks(self, nested_predicate_type="type-n", tables=None, candidate_num=1):
        candidate_inner_query_blocks = []
        if tables is None:
            tables = self.all_table_set
        
        if self.args.is_prov:
            tables = [table for table in tables if table not in self.prov_query_fixer.highlighter.missing_tables]
            if len(tables) == 0:
                tables = self.all_table_set
        
        table_to_single_query_blocks = self.table_to_single_query_blocks if nested_predicate_type in ["type-n", "type-j"] else self.table_to_single_query_blocks_w_agg
        for table in tables:
            if table in table_to_single_query_blocks:
                candidate_inner_query_blocks.extend(table_to_single_query_blocks[table])
        
        if nested_predicate_type in ["type-a", "type-ja"]:
            candidate_inner_query_blocks = [block for block in candidate_inner_query_blocks if "count" not in block.lower()]
        
        candidate_num = min(candidate_num, len(candidate_inner_query_blocks))
        candidate_inner_query_blocks = self.rng.choice(candidate_inner_query_blocks, size=candidate_num, replace=False)
        return candidate_inner_query_blocks

    def generate_nested_predicate(self, nested_predicate_type, accessed_tables, selected_inner_query_block, generated_where_clause, generated_from_clause):
        schema = self.get_schema(accessed_tables)
        system_prompt, user_prompt = self.get_prompt("nested_predicate", schema, nested_predicate_type=nested_predicate_type, selected_inner_query_block=[selected_inner_query_block], generated_where_clause=[generated_where_clause], generated_from_clause=[generated_from_clause])
        is_error, response = self.call_llm(system_prompt, user_prompt, clause_type="nested_predicate", max_tokens=512)
        if is_error:
            return None, None

        generated_nested_predicate = post_process_nested_predicate(response['nested_predicate'][0])
    ...
    def get_prompt(self, clause_type, schema="", history=[], selected_inner_query_block=[], candidate_inner_query_blocks=None, nested_predicate_type="", generated_where_clause="", generated_from_clause="", query="", provenance_analysis_results="", problematic_subquery="", problematic_condition="", error_log="", problematic_subquery_execution_result=""):
        if clause_type == "nested_predicate":
            if nested_predicate_type == "type-n":
                system_prompt = self.nested_predicate_prompt_temp.system_generate_nested_predicate_type_n_prompt.format(height=self.args.hyperparams["height"])
            elif nested_predicate_type == "type-j":
                system_prompt = self.nested_predicate_prompt_temp.system_generate_nested_predicate_type_j_prompt.format(height=self.args.hyperparams["height"])
            elif nested_predicate_type == "type-a":
                system_prompt = self.nested_predicate_prompt_temp.system_generate_nested_predicate_type_a_prompt.format(height=self.args.hyperparams["height"])
            elif nested_predicate_type == "type-ja":
                system_prompt = self.nested_predicate_prompt_temp.system_generate_nested_predicate_type_ja_prompt.format(height=self.args.hyperparams["height"])
            user_prompt = self.nested_predicate_prompt_temp.user_generate_nested_predicate_prompt.format(schema=schema, generated_from_clause=generated_from_clause, generated_where_clause=generated_where_clause, selected_inner_query_block=selected_inner_query_block)
        
        elif clause_type == "inner_query_block":
            system_prompt = self.nested_predicate_prompt_temp.system_select_inner_query_block_prompt
            user_prompt = self.nested_predicate_prompt_temp.user_select_inner_query_block_prompt.format(schema=schema, generated_from_clause=generated_from_clause, generated_where_clause=generated_where_clause, candidate_inner_query_blocks=candidate_inner_query_blocks)
        
        elif clause_type == "from":
            system_prompt = self.nested_predicate_prompt_temp.system_from_prompt
            user_prompt = self.nested_predicate_prompt_temp.user_from_prompt.format(schema=schema, subquery=selected_inner_query_block)
        elif clause_type == "execution_error":
            system_prompt = self.correction_prompt_temp.system_execution_error_correction_prompt
            user_prompt = self.correction_prompt_temp.user_execution_error_correction_prompt.format(query=query, error_log=error_log, schema=schema)
        elif clause_type == "empty_result_error":
            system_prompt = self.correction_prompt_temp.system_empty_result_correction_prompt
            user_prompt = self.correction_prompt_temp.user_empty_result_nested_predicate_correction_prompt.format(query=query, provenance_analysis_results=provenance_analysis_results, problematic_subquery=problematic_subquery, problematic_condition=problematic_condition, problematic_subquery_execution_result=problematic_subquery_execution_result)
        else:
            system_prompt, user_prompt = super().get_prompt(clause_type, schema, history)
        
        return system_prompt, user_prompt

    def verify_nested_predicate_with_database(self, outer_tables, generated_nested_predicate, logical_operator, generated_where_clause):
        error_type = None
        ...
        if "." in outer_column:
            outer_tables = list(set([outer_column.split(".")[0]]).union(set(outer_tables)))
        
        outer_tables = [table for table in outer_tables if table in self.all_table_set]
        
        if len(outer_tables) > 1:
            from_clause = "FROM " + (
                generate_from_clause_for_full_outer_view(self.args, relations=outer_tables, join_clause_list=self.join_clause_list, is_inner_join=True)
            )
        else:
    ...
    def correct_query(self, error_type, query, error_log, nested_predicate_type, accessed_tables=None, outer_table=None):
        if error_type in ["execution_error", "nested_predicate_type_mismatch"]:
            ast = parse_one(query)
            ast.set("expressions", [exp.Star()])
            query = str(ast)
            if error_type == "nested_predicate_type_mismatch" and "type-j" in nested_predicate_type:
                query = query.replace(f"SELECT * FROM {outer_table}", f"SELECT * FROM {outer_table} AS T1 ")
            schema = self.get_schema(accessed_tables)
            system_prompt, user_prompt = self.get_prompt("execution_error", query=query, error_log=error_log, schema=schema)
        elif error_type == "empty_result_error":
            try:
                provenance_analysis_results, problematic_condition, problematic_subquery, problematic_subquery_execution_result = self.prov_query_fixer.fix_query(query, nested_predicate_type)
            except Exception as e:
                return None
            ...
            ast.set("expressions", [exp.Star()])
            query = str(ast)
            system_prompt, user_prompt = self.get_prompt(error_type, query=query, provenance_analysis_results=provenance_analysis_results, problematic_subquery=problematic_subquery, problematic_condition=problematic_condition, problematic_subquery_execution_result=problematic_subquery_execution_result)
        
        is_error, response = self.call_llm(system_prompt, user_prompt, clause_type="corrected_query", max_tokens=256)
        ...
        if error_type in ["execution_error", "nested_predicate_type_mismatch"]:
            table_to_alias = from_clause_to_dict(parse_one(corrected_query).args.get("from"))
            alias_to_table = {v: k for k, v in table_to_alias.items()}
            for alias, table in alias_to_table.items():
                nested_predicate = nested_predicate.replace(alias, table).replace(f"AS {table}", "")
        
        return nested_predicate

    def get_error_log_for_nested_predicate(self, generated_nested_predicate, stype_set, gt_type_set):
        try:
        ...
        if correct_type == "type-ja":
            error_log += f"{self.nested_predicate_type_prompt.type_ja_predicate_requirement}\n"
        elif correct_type == "type-j":
            error_log += f"{self.nested_predicate_type_prompt.type_j_predicate_requirement}\n"
        elif correct_type == "type-a":
            error_log += f"{self.nested_predicate_type_prompt.type_a_predicate_requirement}\n"
        elif correct_type == "type-n":
            error_log += f"{self.nested_predicate_type_prompt.type_n_predicate_requirement}\n"
        ...
        if correct_type == "type-ja":
            ...
            error_log += f"{self.nested_predicate_type_prompt.type_a_predicate_format}\n\n"
        elif correct_type == "type-n":
            error_log += f"{self.nested_predicate_type_prompt.type_n_predicate_format}\n\n"
