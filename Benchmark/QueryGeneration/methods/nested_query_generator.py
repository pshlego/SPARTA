    ...
    def nest_inner_query_block(
        ...
        while (
            ...
            if self.args.hyperparams["height"] > 2:
                if set(stype_list[:1]) != gt_type_set or len(stype_list) != len(
                    self.args.hyperparams["nested_predicate_types"]
                ) + (self.args.hyperparams["height"] - 1):
                    nested_predicate_gen_count += 1
                    self.logger.error(
            ...
            while (
                ...
                if self.args.is_prov:
                    ...
                    if self.args.hyperparams["height"] > 2:
                        ...
                        gt_type_set = set(
                            self.args.hyperparams["nested_predicate_types"][
                                : child_query_id + (self.args.hyperparams["height"] - 1)
                            ]
                        )
                    ...
                    if is_strange_type_2 or is_strange_type_1:
                        ...
                        if error_type is None:
                            ...
                            if set(stype_list[:1]) != gt_type_set or len(
                                stype_list
                            ) != len(
                                self.args.hyperparams["nested_predicate_types"]
                            ) + (
