from M_Schema.utils import examples_to_str, read_json, write_json
from typing import Any, Dict, List, Optional


class MSchema:
    def __init__(self, db_id: str = 'Anonymous', schema: Optional[str] = None, rng = None):
        self.db_id = db_id
        self.schema = schema
        self.tables = {}
        self.foreign_keys = []
            ...
            if len(field_info.get('examples', [])) > 0 and example_num > 0:
                ...
                if len(examples) > example_num:
                    if self.rng is not None:
                        examples = self.rng.choice(examples, size=example_num, replace=False)
                    else:
