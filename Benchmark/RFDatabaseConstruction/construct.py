...
def create_stat_table(dbname, port):
    con = psycopg2.connect(f"host=localhost dbname = {dbname} user=postgres password=postgres port = {port}")
    cursor = con.cursor()
    ...
    for line in lines:
        ...
        if section == "Team" and not team_headers:
            ...
            def construct_data_dict(header, values):
                ...
                filtered_values = [values[i] for i, key in enumerate(header) if key != exclude_keys]
...
def insert_stat_table(grounding_data_dir_path, dbname, port):
    ...
    for split in ["train", "valid", "test"]:
        ...
        with open(f"{grounding_data_dir_path}/{split}.data", 'r') as data_file, open(f"{grounding_data_dir_path}/{split}.txt", 'r') as data_txts:
...
def add_summary_id(dbname, port):
    ...
    con = psycopg2.connect(f"host=localhost dbname = {dbname} user=postgres password=postgres port = {port}")
    cursor = con.cursor()
    tables = [ "prep_team_stats", "prep_person_stats", "prep_game_information", "prep_team_info", "prep_next_info"]
...
def link_two_databases(sql_file_path, dbname, port):
    conn = psycopg2.connect(f"host=localhost dbname = {dbname} user=postgres password=postgres port = {port}")
    cur = conn.cursor()
...
def rename_columns_in_db(table_name, cursor):
    ...
    for column in columns:
        ...
        if new_column != column:
            if new_column == 'to':
                new_column = 'turnover'
            cursor.execute(f"ALTER TABLE {table_name} RENAME COLUMN \"{column}\" TO {new_column}")
...
def create_btree_indexes(cur):
    ...
    for table in tables:
        ...
        for column in columns:
            ...
            try:
                cur.execute(f"CREATE INDEX {index_name} ON {table} USING btree({column});")
...
def create_hash_indexes_on_foreign_keys(cur):
    ...
    for column_name, table_name in cur.fetchall():
        ...
        try:
            cur.execute(f"CREATE INDEX {index_name} ON {table_name} USING hash({column_name});")
...
def rename_columns(sql_file_path, dbname, port):
    ...
    cur.execute("ALTER TABLE game_information ADD CONSTRAINT game_information_summary_id_fk FOREIGN KEY (summary_id) REFERENCES summary(summary_id);")
    cur.execute("ALTER TABLE game_information ADD CONSTRAINT home_team_id_fk FOREIGN KEY (home_team_id) REFERENCES nba_team_information(team_id);")
    cur.execute("ALTER TABLE game_information ADD CONSTRAINT visitor_team_id_fk FOREIGN KEY (visitor_team_id) REFERENCES nba_team_information(team_id);")
...
def add_date(dbname, port):
    ...
    cur = conn.cursor()
    cur.execute("SELECT column_name FROM information_schema.columns WHERE table_name='game_information' and column_name='game_date';")
    if not cur.fetchone():
    ...
    for row in rows:
        ...
        updated_first_sentence = formatted_date_str + ' ' + first_sentence[0].lower() + first_sentence[1:]
...
def delete_duplicate_name(dbname, port, use_cascade=True):
    ...
    for conname, table_name, constraint_def in constraints:
        if use_cascade:
            drop_sql = f"ALTER TABLE {table_name} DROP CONSTRAINT IF EXISTS {conname} CASCADE;"
        else:
    ...
    cur.execute("ALTER TABLE nba_champion_history DROP COLUMN western_champion CASCADE;")
...
@hydra.main(config_path = "conf", config_name = "construct")
def main(cfg: DictConfig):
    args = cfg
    source_data_dir_path = args.source_data_dir_path
    grounding_data_dir_path = args.grounding_data_dir_path
    join_file_path = args.join_file_path
    rename_file_path = args.rename_file_path
