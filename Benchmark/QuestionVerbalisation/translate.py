...
from Benchmark.QueryGeneration.methods.utils import PostgreSQLDatabase
...
def convert_dates_to_isoformat(lst):
    return [elem.isoformat() if isinstance(elem, datetime.date) else elem for elem in lst]

@hydra.main(config_path = "conf", config_name = "translate")
def main(cfg: DictConfig):
    with open(cfg.dataset_test, "r") as fp:
        single_query_blocks = fp.readlines()

    data_manager = PostgreSQLDatabase("postgres", "postgres", "localhost", cfg.port, cfg.db_name)
    df = pd.read_csv(cfg.examples_path)
    prompts_all = df['prompt'].tolist()
    llama_sampling_params = {"temperature": cfg.temperature, "top_p": cfg.top_p, "max_new_tokens": cfg.max_new_tokens }
    translation_data = []
    ...
    final_data = []
    count = 0
    for translation_datum, single_query_block in zip(translation_data, single_query_blocks):
        if "order by" in single_query_block.lower():
            modified_single_query_block = single_query_block
        else:
            modified_single_query_block = "SELECT DISTINCT " + single_query_block[len("SELECT "):]
        data_manager.execute(modified_single_query_block)
        execution_results = data_manager.fetchall()
        answer = convert_dates_to_isoformat([decimal_default(execution_result[0]) for execution_result in execution_results])
        final_data.append({"question_id": count, "question": translation_datum, "sql_query": single_query_block, 'answer': answer})
        count += 1
