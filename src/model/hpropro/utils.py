...
import json
import inspect
import src.model.hpropro.code_template as code_template
from src.model.hpropro.code_template import *
from datetime import date, datetime, timedelta
from src.prompt.model.hpropro.ans_post_process import ANS_POST_PROCESS_PROMPT
from src.model.hpropro.query_api import query_API
import os


def execute_generated_code(
    generated_code, table_pd_list, multi_table, adapter, example
):
    ...
    if multi_table:
        exec_ctx = {
            "tables": table_pd_list,
            "passage_store": adapter.get_passage_store(example),
        }
    else:
        exec_ctx = {
            "table": table_pd_list,
    ...
    ans = safe_execute(code, exec_ctx)
...
def create_few_shot_code_prompt(shot_num, few_shot_case_path, mode="pot"):
    """Create few-shot code prompt from case files.

    Args:
        shot_num (int): The num of the cases in the prompt in the few-shot settings.
        few_shot_case_path (str): Path to the few_shot_case directory.
    ...
    case_file_link = os.path.join(
        few_shot_case_path, "code_few_shot_cases_w_check.json"
    )
...
def ans_post_process(query, ans_raw):
    ...
    ans_processed = query_API(ans_process_prompt, model="gpt-3.5-turbo")
    return ans_processed


def refine(old_prompt, code_w_err, err_info, model):
...
def major_voting(ans_list: list) -> str:
    ...
    sorted_dict = dict(
        sorted(result_set.items(), key=lambda item: item[1], reverse=True)
    )
    print(sorted_dict)
