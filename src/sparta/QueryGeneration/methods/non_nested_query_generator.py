...
from src.prompt.sparta.QueryGeneration.non_nested import (
    oneshot,
    cbc,
    eg_movie,
    eg_medical,
...
from src.benchmark.sparta.QueryGeneration.methods.M_Schema.schema_engine import (
    SchemaEngine,
)
...
class OneShotQueryGenerator:
    def __init__(self, args, data_manager, rng, all_table_set):
        ...
        self.sampling_params = {
            "temperature": self.args.llm_hyperparams["temperature"],
            "top_p": self.args.llm_hyperparams["top_p"],
            "max_new_tokens": self.args.llm_hyperparams["max_tokens"],
        }
        self.mschema = SchemaEngine(
            engine=create_engine(
                f"postgresql+psycopg2://postgres:postgres@localhost:{self.args.port}/{self.args.db}"
            ),
    ...
    def call_llm(self, system_prompt, user_prompt, clause_type=None, max_tokens=None):
        ...
        print(
            f"ideal_calls: {self.ideal_calls} total_calls: {self.total_calls} error_dict: {self.error_dict}"
        )
    ...
    def detect_error(self, response, clause_type=None):
        ...
        for sql in sql_queries:
            if sql in self.generated_sql_query_set or is_new_query_equivalent(
                list(self.generated_sql_query_set), sql
            ):
            ...
            try:
                ...
                if (
                    result_len == 1
                    and isinstance(first_result, int)
                    and first_result == 0
                ):
                    self.error_dict["empty"] += 1
                    continue
...
class CBCQueryGenerator(OneShotQueryGenerator):
    def __init__(
        self, args, data_manager, rng, all_table_set, table_info, dtype_dict, logger
    ):
    ...
    def generate(
        ...
        while True:
            ...
            for clause_type in sql_clause_list:
                revised_clause, relations = self.generate_clause(
                    schema, history, clause_type, generated_clause_info_dict
                )
                if relations:
    ...
    def generate_clause(
        self, schema, history, clause_type, generated_clause_info_dict=None
    ):
        if clause_type == "from":
            initial_relation = self.choose_initial_relation()
            ...
            group_by_column_list = generated_clause_info_dict[
                "columns_in_group_by_clause"
            ]
        ...
        is_error, response = self.call_llm(
            system_prompt, user_prompt, clause_type=clause_type
        )
        if not is_error:
            revised_response = self.revise_clause(response, clause_type)
            self.update_generated_clause_info_dict(
                generated_clause_info_dict, clause_type, revised_response
            )
    ...
    def choose_initial_relation(
        ...
        df_columns = sorted(
            [
                table + "." + column
                for table in self.all_table_set
        ...
        if self.sql_type_dict["group"]:
            ...
            if len(candidate_root_tables) == 0:
                self.logger.warning(
                    "No table has a categorical column but we will generate GROUP BY clause"
                )
    ...
    def update_generated_clause_info_dict(
        self, generated_clause_info_dict, clause_type, revised_clause, relations=None
    ):
        generated_clause_info_dict[clause_type] = revised_clause
        if relations:
            ...
            for relation in generated_clause_info_dict["relations"]:
                ...
                for column in columns:
                    if len(generated_clause_info_dict["relations"]) > 1:
                        ...
                        if f".{column.lower()}" in revised_clause.lower().replace(
                            "__", "."
                        ) or (
                            "." not in revised_clause.lower().replace("__", ".")
                            and column.lower()
                            in revised_clause.lower().replace("__", ".")
                        ):
    ...
    def revise_clause(self, generated_clause, clause_type):
        ...
        clause_value = (
            generated_clause.get(clause_type, "")[0]
            .replace(
        ...
        if clause_type == "select":
            clause_value = (
                clause_value.split("FROM")[0]
                .split(keywords[clause_type])[-1]
                .replace(
                    "number_of_three_point_field_goals_attemp ",
                    "number_of_three_point_field_goals_attempted ",
                )
    ...
    def get_prompt(self, clause_type, schema, history):
        ...
        user_prompt = self.prompt_temp.user_prompt.format(
            schema=schema, history=history
        )
    ...
    def detect_error(self, response, clause_type=None):
        if clause_type is not None:
            ...
            if response["sql"][
                0
            ] in self.generated_sql_query_set or is_new_query_equivalent(
                list(self.generated_sql_query_set), response["sql"][0]
            ):
                self.error_dict["duplicate"] += 1
                return True, response
            try:
        ...
        for clause in ["select", "from", "where", "group", "having", "order", "limit"]:
            if clause in generated_clause_info_dict:
                if generated_clause_info_dict[clause] != "":
                    sql_query += (
                        f"{generated_clause_info_dict[clause].replace('__', '.')} "
                    )
        try:
...
class EGQueryGenerator(CBCQueryGenerator):
    def __init__(
        ...
        super().__init__(
            args, data_manager, rng, all_table_set, table_info, dtype_dict, logger
        )
        if args.domain == "movie":
            self.prompt_temp = eg_movie
        elif args.domain == "medical":
            self.prompt_temp = eg_medical
    ...
    def get_instances(self, query_info, clause_type):
        if clause_type == "from":
            if len(query_info["relations"]) > 1:
                ...
                select_clause = generate_select_clause_for_full_outer_view(
                    self.table_info, relations=query_info["relations"], use_alias=True
                )
                ...
                select_clause = generate_select_clause_for_full_outer_view(
                    self.table_info, relations=query_info["relations"], use_alias=False
                )
                full_inner_join_sql = f"""
            ...
            return self.create_and_sample_view(
                "accessed_tables", full_inner_join_sql, is_materialized=True
            )
            ...
            return self.data_manager.sample_rows_with_cond(
                "accessed_tables", cond, 5, rng=self.rng
            )
            ...
            if len(query_info["relations"]) > 1:
                ...
                for col in group_by_column_list:
                    filtered_group_by_column_list.append(
                        f"{col.split('.')[0]}__{col.split('.')[1]}"
                    )
                ...
                having_candidate_columns = [
                    col for col in column_names if col not in group_by_column_list
                ]
            else:
                having_candidate_columns = [
                    f"{query_info['relations'][0]}.{col}"
                    for col in column_names
                    if col not in group_by_column_list
                ]
                ...
                candidate_aggs = (
                    ...
                    if dtype in ["bool", "str"]
                        ...
                        if dtype == "date"
                            ...
                            or is_hashcode_column(
                                self.args, column_name.replace("__", ".")
                            )
                            else NUMERIC_AGGS
                        )
                    )
                )
                having_candidate_columns_with_agg.extend(
                    (agg, column_name.split(".")[-1]) for agg in candidate_aggs
                )

            select_clause = ", ".join(
                group_by_column_list
                + [
                    f"{agg}({col.replace('.', '__')}) as {agg.lower()}__{col.replace('.', '__')}".replace(
                        "count__", "num__"
                    )
                    for agg, col in having_candidate_columns_with_agg
                ]
            )
            if len(query_info["relations"]) > 1:
                query_info["group"] = replace_dot(query_info["group"])
            else:
                query_info["group"] = query_info["group"].replace(
            ...
            if clause_type == "having":
                if len(query_info["relations"]) > 1:
                    ...
                    query_info["having"] = query_info["having"].replace(
                        f"{query_info['relations'][0]}.", ""
                    )
                sql += f" {query_info.get('having', '')}"

            sample_rows, sample_schema = self.create_and_sample_view(
                f"accessed_tables_with_{clause_type}", sql
            )
            sample_schema = group_by_column_list + [
                f"{col.split('__')[0].upper()}({'__'.join(col.split('__')[1:])})".replace(
                    "NUM(", "COUNT("
                )
            ...
            if clause_type == "order":
                if len(query_info["relations"]) > 1:
                    ...
                    query_info["order"] = query_info["order"].replace(
                        f"{query_info['relations'][0]}.", ""
                    )
            elif clause_type == "limit":
                if len(query_info["relations"]) > 1:
                    ...
                    query_info["limit"] = query_info["limit"].replace(
                        f"{query_info['relations'][0]}.", ""
                    )
    ...
    def get_sql_query(self, generated_clause_info_dict, consider_duplicate=True):
        ...
        for clause in ["select", "from", "where", "group", "having", "order", "limit"]:
            ...
            elif clause == "select" and not consider_duplicate:
                sql_query += "SELECT * "
        ...
        if not consider_duplicate:
            return [sql_query]

        is_error = False
        if sql_query in self.generated_sql_query_set or is_new_query_equivalent(
            list(self.generated_sql_query_set), sql_query
        ):
            self.error_dict["duplicate"] += 1
            is_error = True
        else:
    ...
    def remove_all_none_columns(self, rows, column_names):
        ...
        valid_indices = [
            i
            for i, col in enumerate(transposed_columns)
            if not all(val is None for val in col)
        ]
        ...
        for i, valid_index in enumerate(valid_indices):
            ...
            if (
                "avg__" in valid_column_name.lower()
                or "sum__" in valid_column_name.lower()
                or "count__" in valid_column_name.lower()
                or "min__" in valid_column_name.lower()
                or "max__" in valid_column_name.lower()
            ):
                filtered_column_names.append(
                    f"{valid_column_name.split('__')[0].upper()}({valid_column_name.split('__')[-1]})"
                )
            else:
