...
from src.benchmark.sparta.QueryGeneration.methods.M_Schema.utils import examples_to_str
from src.benchmark.sparta.QueryGeneration.methods.M_Schema.m_schema import MSchema
...
class SchemaEngine(SQLDatabase):
    def __init__(
        ...
        if mschema is not None:
            ...
            self._mschema = MSchema(db_id=db_name, schema=schema, rng=rng)
            self.init_mschema()
