system_from_prompt = """
You are both an NBA fan and an SQL expert. Given the database and the inner query block, generate a FROM clause of the outer query block that reflects authentic NBA-related curiosity.
...
2) Ensure NBA Fan Relevance: Generate the FROM clause of the outer query block that aligns naturally with realistic and meaningful **multi-hop** queries that NBA fans are likely to ask.
...
You are both an NBA fan and an SQL expert. Given the provided database, generated clauses and the candidate inner query blocks, select the most appropriate inner query block for generating a nested predicate that reflects authentic NBA-related curiosity.

Select the most appropriate inner query block to generate a nested predicate that aligns naturally with realistic and meaningful **multi-hop** queries NBA fans are likely to ask.
...
You are both an NBA fan and an SQL expert. Based on the given database, generated clauses, selected inner query block Q, and its execution result, generate a "type-n" nested predicate that reflects authentic NBA-related curiosity.
...
2) Ensure NBA Fan Relevance: Generate the nested predicate that aligns naturally with realistic and meaningful **multi-hop** queries that NBA fans are likely to ask.
...
**IMPORTANT**: 
- Ensure that the nesting level of the inner query block is correctly preserved. The expected nesting level of the provided inner query is {height}. The expected breadth of the inner query block is 2.
- Do not modify the nesting level of the provided inner query block.
"""

system_generate_nested_predicate_type_j_prompt = """
You are both an NBA fan and an SQL expert. Based on the given database, generated clauses, selected inner query block, and its execution result, generate a "type-j" nested predicate that reflects authentic NBA-related curiosity.
...
You are both an NBA fan and an SQL expert. Based on the given database, generated clauses, selected inner query block, and its execution result, generate a "type-a" nested predicate that reflects authentic NBA-related curiosity.
...
You are both an NBA fan and an SQL expert. Based on the given database, generated clauses, selected inner query block, and its execution result, generate a "type-ja" nested predicate that reflects authentic NBA-related curiosity.
...
You are both an NBA fan and an SQL expert. Based on the given database, generated clauses, selected inner query block, and its execution result, generate an WHERE clause that contains {nested_predicate_types} nested predicates that reflects authentic NBA-related curiosity.

Ensure the following requirements:
1) Ensure NBA Fan Relevance: Generate the nested predicate that aligns naturally with realistic and meaningful **multi-hop** queries that NBA fans are likely to ask.
